...
with aba_avaliacoes:
            ...
            if q_sel != "Escolha uma questão...":
                ...
                if q_data:
                    ...
                    if cb_s.button("💾 Salvar Alterações", type="primary", use_container_width=True):
                        ...
                        c.execute('''UPDATE questoes SET disciplina=?, assunto=?, dificuldade=?, enunciado=?, pontos=?, espaco_resposta=?, espaco_linhas=?, tipo=?, imagem=?, gabarito_imagem=?, gabarito_discursivo=? WHERE id=?''', (n_disc, n_ass, n_dif, n_enun_final, n_pts, n_esp, n_tam, n_tipo, i_f, i_g_f, val_gab, id_editar))
                    ...
                    if cb_d.button("💾 Salvar como Nova", use_container_width=True, help="Cria uma CÓPIA exata no banco (ideal para mudar valores)"):
                        ...
                        inserir_questao(n_disc, n_ass, n_dif, n_enun_final, alts_para_inserir, n_pts, n_tipo, val_gab, i_f, n_esp, n_tam, i_g_f)
    ...
    with sub_gen:
        ...
        if modo_selecao == "Sorteio Automático":
            ...
            # Criamos uma caixa com borda para agrupar as regras
            with st.container(border=True):
                # 1. Cabeçalho das colunas (aparece apenas uma vez no topo)
                c_head1, c_head2, c_head3, c_head4 = st.columns([0.15, 0.35, 0.25, 0.25])
                c_head1.markdown("**Qtd**")
                c_head2.markdown("**Assunto**")
                c_head3.markdown("**Dificuldade**")
                c_head4.markdown("**Tipo**")

                regras = []
                # 2. Laço de repetição das regras (sem repetir os títulos)
                for i in range(st.session_state.num_regras):
                    ...
                    regras.append({
                        ...
                        "tipo": c4.selectbox(f"Tip_{i}", ["Todos", "Múltipla Escolha", "Verdadeiro ou Falso", "Discursiva", "Numérica"], key=f"r_tip_{i}", label_visibility="collapsed")
                    })
                
                # 3. Botões de Adicionar/Remover menores e alinhados dentro da caixa
                st.write("") # Leve respiro
            ...
            # 4. Botão de Sortear com destaque total
                    ...
                    for idx_img, img_orig in enumerate(imagens_para_processar):
                        try:
                            ...
                            if st.button(f"💾 Confirmar e Salvar: {dados_qr['nome']}", key=f"sv_{idx_img}"):
                                # 1. Salva a nota final no Boletim Mestre
                                
                                
                                # 2. Salva o feedback de cada questão para o Portal do Aluno
                                for r in resumos:
...
with aba_turmas:
    with sqlite3.connect('banco_provas.db') as conn:
        ...
        if id_t_ativa and d_ativa:
                ...
                # =========================================================================
                # ✏️ 3. AVALIAÇÕES E LANÇAMENTO DE NOTAS (SISTEMA DE 4 ABAS)
                # =========================================================================
                with sub_pesos:
                    # --- 0. PREPARAÇÃO DE DADOS (Resolve o NameError) ---
                    ...
                    # --- ABA 4: CONFIGURAÇÕES (Onde você define Provas, Listas e Labs) ---
                    with t_config:
                        st.info(f"⚡ Atualmente existem {qtd_aulas_registradas} aulas registradas no Dojo.")
                        ...
                        if st.button("🔄 Aplicar e Recalcular Médias", use_container_width=True):

                            # 1. Monta os nomes das colunas
                            cols_provas = [f"{n_p} {i+1}" for i in range(int(q_p))]      
                            cols_listas = [f"{n_l} {i+1}" for i in range(int(q_l))]      
                            cols_lab    = [f"{n_lb} {i+1}" for i in range(int(q_lb))]    
                            bloco_ativ  = f"{n_a} (Dojo)"                                

                            # ✅ A GRANDE CORREÇÃO: Transformar o ID do Pandas em Inteiro Puro do Python!
                            ...
                            for nome in cols_provas:
                                conn.execute("INSERT INTO planejamento_notas (turma_id, disciplina, nome_avaliacao, peso) VALUES (?,?,?,?)", (id_turma_seguro, d_ativa, nome, float(w_p) / max(1, int(q_p))))

                            for nome in cols_listas:
                                conn.execute("INSERT INTO planejamento_notas (turma_id, disciplina, nome_avaliacao, peso) VALUES (?,?,?,?)", (id_turma_seguro, d_ativa, nome, float(w_l) / max(1, int(q_l))))

                            for nome in cols_lab:
                                conn.execute("INSERT INTO planejamento_notas (turma_id, disciplina, nome_avaliacao, peso) VALUES (?,?,?,?)", (id_turma_seguro, d_ativa, nome, float(w_lb) / max(1, int(q_lb))))

                            conn.execute("INSERT INTO planejamento_notas (turma_id, disciplina, nome_avaliacao, peso) VALUES (?,?,?,?)", (id_turma_seguro, d_ativa, bloco_ativ, float(w_a)))
                            
                            # Efetiva no banco
                            conn.commit()

                            st.success("Planejamento salvo com sucesso no banco de dados!")
                            st.rerun()
                    ...
                    if not df_alunos.empty:
                        ...
                        m_p, m_l, m_lb, m_a = f"Média {n_p}", f"Média {n_l}", f"Média {n_lb}", f"Média {n_a}"
                        df_calc[m_p] = df_calc.apply(lambda r: calc_media(r, cols_provas, c_p), axis=1)
                        df_calc[m_l] = df_calc.apply(lambda r: calc_media(r, cols_listas, c_l), axis=1)
                        df_calc[m_lb] = df_calc.apply(lambda r: calc_media(r, cols_labs, 0), axis=1)
                        df_calc[m_a] = df_calc.apply(lambda r: calc_media(r, cols_ativs, c_a), axis=1)
                        df_calc["MÉDIA FINAL"] = ((df_calc[m_p]*(w_p/100))+(df_calc[m_l]*(w_l/100))+(df_calc[m_lb]*(w_lb/100))+(df_calc[m_a]*(w_a/100))).round(2)

                        # --- CONTEÚDO DAS OUTRAS ABAS ---
                        with t_manual:
                            st.caption("As colunas de 'Ativ' são automáticas do Dojo. Duplo clique para editar.")
                            colunas_display = ["Matrícula", "Nome"] + todas_cols_entrada + ["MÉDIA FINAL"]
                            df_ed = st.data_editor(df_calc[colunas_display], use_container_width=True, hide_index=True, key="ed_notas_vFinal")
                        ...
                        with t_excel:
                            ...
                            st.file_uploader("Subir arquivo:", type=["xlsx", "csv"], key="up_notas_excel")

                        with t_auto:
                            st.markdown("#### 🦾 Corretor Automático")
                            sel_p = st.selectbox("Selecione a prova:", cols_provas if cols_provas else ["Nenhuma"], key="sel_prova_auto")
                            if st.button("🔄 Sincronizar com o Robô"): st.warning("Conectando ao banco de correções...")
            ...
            if t_aula_nome != "-- Escolha --":
                ...
                if disc_sel != "-- Escolha --":
                    ...
                    with col_t2:
                        ...
                        st.components.v1.html("""<div style="text-align: center; background: #f0f2f6; padding: 10px; border-radius: 10px;"><canvas id="meter" width="300" height="40"></canvas><div id="st" style="font-size:12px; font-weight:bold; margin-top:5px;">Microfone Desligado</div></div><script>navigator.mediaDevices.getUserMedia({audio:true}).then(s=>{const ac=new AudioContext();const an=ac.createAnalyser();const mic=ac.createMediaStreamSource(s);mic.connect(an);const d=new Uint8Array(an.frequencyBinCount);const can=document.getElementById('meter');const ctx=can.getContext('2d');function draw(){an.getByteFrequencyData(d);let av=d.reduce((a,b)=>a+b)/d.length;ctx.clearRect(0,0,300,40);ctx.fillStyle=(av>50?"#e74c3c":"#2ecc71");ctx.fillRect(0,0,(av/100)*300,40);document.getElementById('st').innerText=(av>50?"🛑 Silêncio!":"✅ OK");requestAnimationFrame(draw);}draw();})</script>""", height=100)

                    st.write("---")
                    modo_aula = st.radio("Selecione a ação:", ["⭐ Comportamento", "🙋 Fazer Chamada", "✍️ Atividade de Sala", "🎲 Sortear Aluno", "👥 Grupos", "📖 Registrar Diário"], horizontal=True, key="radio_modo_final")
                    ...
                    if modo_aula == "⭐ Comportamento":
                        ...
                        with cols[0]:
                            with st.container(border=True):
                                st.markdown(f"<div style='text-align:center; height:140px;'><h3>🌍</h3><b>Turma</b><br><small style='color:green;'>●</small>{int(df_t_pts['d_pos'])}|<small style='color:red;'>●</small>{int(abs(df_t_pts['d_neg']))}<br><small>⭐{int(df_t_pts['t_geral'])}</small></div>", unsafe_allow_html=True)
                                if st.button("Feedback", key="bt_t"): modal_feedback('TURMA_INTEIRA', 'Toda a Turma')
                        ...
                        for idx, row in alunos_dojo.iterrows():
                            with cols[(idx+1)%6]:
                                with st.container(border=True):
                                    ...
                                    if st.button("Feedback", key=f"f_{row['ra']}"): modal_feedback(row['ra'], row['nome'])
                    ...
                    # =========================================================
                        ...
                        # ✨ MUDANÇA AQUI: Usar col_sel em vez de st
                        ...
                        if not st.session_state.gs_m:
                            t_f = st.radio("Tipo de formação:", ["Aleatório", "Manual"])

                            if t_f == "Aleatório":
                                n_g = st.number_input("Tamanho dos grupos:", 1, 10, 4)
                                ...
                                st.info("Formação manual ainda não implementada nesta versão.")
                        else:
