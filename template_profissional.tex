...
\usepackage{enumitem}         % Ajuda a controlar melhor as listas se necessário
...
<% for q in questoes %>
    ...
    <% if q.imagem %>
    \begin{center}
        \includegraphics[max width=0.95\linewidth, max height=8cm, keepaspectratio]{<< q.imagem >>}
    \end{center}
    <% endif %>
    ...
    <% if q.tipo == "Discursiva" or q.tipo == "Numérica" %>
        \vspace{0.3cm}
        <% if q.espaco == "Linhas" %>
            \fillwithlines{<< q.espaco_linhas >>cm}
